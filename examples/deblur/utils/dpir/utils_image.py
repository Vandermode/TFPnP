...
# =======================================
'''
def get_image_paths_dict(dataroot, directories=None):
    """ datatype='H': dataroot only contains ground truth image 
        datatype='HL': dataroot contains both gt and low quality images, 
                       where gt lies in 'gt' subdierctory, and low quality 
                       image lies in 'low' subdirectory
    """
    ...
    if dataroot is not None:
        if directories is None:
            ...
            for d in directories:
                ds[d] = sorted(_get_paths_from_images(os.path.join(dataroot, d)))
                length = len(ds[d])
            paths = []
...
# PSNR
# ----------
def calculate_psnr_01(img1, img2, border=0):
    """ img2 is the reference """
...
def calculate_psnr_y(img1, img2, border=0):
    ...
    psnr_y = calculate_psnr(img1_y, img2_y, border=border)
    return psnr_y
