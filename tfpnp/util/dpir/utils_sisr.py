...
def data_solution_deblur(x, FB, FBC, F2B, FBFy, alpha, sf):
    # data solution for deblur can be simplified as this function.
    FR = FBFy + torch.rfft(alpha*x, 2, onesided=False)
    invWBR = cdiv(FR, csum(F2B, alpha))
    Xest = torch.irfft(invWBR, 2, onesided=False)
    return Xest

# --------------------------------
# Data Solution For ADMM 
# torch.rfft: signal_ndim = 2, 3, both work
# --------------------------------
